...
use Admin\Controllers\RevisionsController;
...
$router->post('/posts/{slug}/revisions/{id}/restore', function (string $slug, int $id): void {
    (new PostsController(PostsRepository::make()))->restoreRevision($slug, $id);
});
...
$router->get('/revisions', function () use ($requireAdmin): void {
    $requireAdmin();
    (new RevisionsController(RevisionsRepository::make(), PostsRepository::make()))->index();
});

$router->get('/posts/{slug}/revisions/{id}', function (string $slug, int $id) use ($requireAdmin): void {
    $requireAdmin();
    (new RevisionsController(RevisionsRepository::make(), PostsRepository::make()))->show($slug, $id);
});

// Post specifieke revisies lijst
$router->get('/posts/{slug}/revisions', function (string $slug) use ($requireAdmin): void {
    $requireAdmin();
    (new RevisionsController(RevisionsRepository::make(), PostsRepository::make()))->showByPost($slug);
});
